...
## シンプルなWebSocketサーバとUI

ローカルで立ち上げて学習・テスト用途に使える小さな WebSocket アプリです。`aiohttp` が HTTP と WebSocket の両方を同じポートで扱い、`public/index.html` から接続・送受信できます。
...
```
python -m pip install -r websocket-server/requirements.txt
```
...
3. ブラウザで `http://localhost:3000/` にアクセス
   ...
   - `ws://localhost:3000/ws` に接続
   - JSON やテキストを送信するとサーバ側でブロードキャストされ、受信ログに反映されます
   - 「標準ポーズを送信」ボタンでロボットアームに渡すような角度データの例を送れます

   - `Ollama生成` フォームに声掛け（例: 「笑って」「手を振って」）を入力すると、サーバが `http://localhost:11434/predict` を呼び出してアニメーション用 JSON を生成し、全接続クライアントに自動ブロードキャストします。生成結果は画面下部のプレビューとログに出力されます。

4. 他クライアント（たとえば `robotarm.html`）を `ws://localhost:3000/ws` に接続すると、同じデータを受け取れます

サーバは `/health` エンドポイントで接続数を返し、10秒ごとにすべての接続に時刻通知を送ります。必要に応じてこのスクリプトを拡張し、実際のロボット制御ロジックに合わせてください。
